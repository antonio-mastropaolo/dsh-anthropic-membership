...
import { ALLOWED_ORIGINS, PORT, apiKeyInEnv, ensureSettings, getAccessToken, login, logout, status } from "./oauth.mjs";
...
async function statusBody() {
  const st = status();
  if (apiKeyInEnv()) {
    return { signedIn: st.signedIn, imageCap: imageCapInstalled, compactionSlim: compactionSlimInstalled, plan: { mode: "api" } };
  }
  if (!st.signedIn) {
    return { signedIn: false, imageCap: imageCapInstalled, compactionSlim: compactionSlimInstalled, plan: { mode: "signed-out" } };
  }
  let plan;
  try {
  ...
  return { signedIn: true, imageCap: imageCapInstalled, compactionSlim: compactionSlimInstalled, plan };
}
