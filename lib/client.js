window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PLAN_CSS = [
      "[role=tablist]:has(.dshAmPlan-host){width:100%;align-items:center}",
      ".dshAmPlan-host{margin-left:auto;display:inline-flex;align-items:center;padding:0 4px 9px 12px}",
      ".dshAmPlan-wrap{display:inline-flex;align-items:center;flex-wrap:nowrap;gap:6px;width:auto;padding:0}",
      ".dshAmPlan{box-sizing:border-box;display:inline-flex;align-items:center;gap:6px;max-width:100%;height:24px;padding:1px 4px 1px 8px;border:.5px solid var(--dsw-alias-border-l1);border-radius:999px;background:var(--dsw-specific-tip,transparent);color:var(--dsw-alias-label-secondary);font-size:11px;font-variant-numeric:tabular-nums;line-height:14px;white-space:nowrap}",
    ...
    function ClaudeChip(plan) {
      ...
      return jsx.jsxs("div", {
        ...
        title: "Claude Pro/Max · 5h and weekly % are account-wide. Anthropic does not publish remaining messages.",
        children: [
          jsx.jsx("span", { className: "dshAmPlan-mark", children: "Claude" }),
    ...
    function DockWidget() {
      ...
      react.useLayoutEffect(() => {
        ...
        const tabs = (header && header.querySelector("[role='tablist']")) || document.querySelector("[role='tablist']");
        if (!tabs) return;
        let host = tabs.querySelector(":scope > .dshAmPlan-host");
        if (!host) {
          host = document.createElement("div");
          host.className = "dshAmPlan-host";
